...
**(2) 서비스 목표**
- 1차적으로 **1만 RPM**(Request Per Minute)을 효과적으로 처리하는 것을 목표로 설정했습니다. 그 이유는 다음과 같습니다:

(2-1) 적정 주기와 전환 인원의 산출: 1만 RPM 기준으로 적절한 주기와 전환 인원을 계산하는 것이 우선적인 목표입니다.
(2-2) 단일 서버 테스트: 현재 단일 서버 환경에서 테스트 중이기 때문에, 1만 RPM 정도가 적절한 수치일 것으로 판단했습니다.

- 실제 인기 콘서트(예: 임영웅 콘서트)의 경우, **최대 370만 명**의 트래픽이 몰릴 수 있습니다. <br>
