...
- 로컬 캐시의 장점은 (1) 빠른 데이터 접근 속도를 제공하고, (2) 캐시 서버의 장애로부터 자유롭다는 점이고, <br> 
  반면, 단점은 분산 환경을 고려할 때, 캐시의 동기화가 복잡할 수 있다는 점입니다. <br>

- 글로벌 캐시의 장점은 분산 환경에서 Single Source of Truth를 재공한다는 점이고, <br>
  반면, 단점은 캐시 서버의 장애로 인한 네트워크 장애가 발생할 수 있다는 점입니다. <br>  

- 그리고 저는 이번 캐시 도입에 있어서는 글로벌 캐시인 Redis를 도입하는 것으로 결정했습니다. <br> 
  그 이유는
