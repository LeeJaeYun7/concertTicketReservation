...
- 서킷 브레이커란, 직역하면 회로 차단기로 서비스간 장애 전파를 막는 역할을 하는 것입니다. <br>
  서킷 브레이커는 문제가 발생했을 때, 이를 감지하고 흐름을 차단함으로써 장애 전파를 막습니다. <br>
...
| 요청에 대한 처리 | 요청에 대한 처리 수행                      | 외부 요청을 차단하고 에러를 뱉거나 지정한 callback 메소드를 호출 | 요청에 대한 처리를 수행하고 실패시 Open 상태로, 성공시 Close 상태로 변경 |
| 상태 변경 기준 | 정해진 횟수/비율만큼 실패할 경우 Open 상태로 변경 | -                                       | -                                        |
