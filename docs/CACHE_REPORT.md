...
```
- **'최근 3일간 판매량 Top30 콘서트'** 정보를 제공하는 데 있어, <br>
  판매량이 지속적으로 업데이트되므로 **실시간 반영**이 필요하다고 판단했습니다. <br> 
  이를 위해 **캐시 스케줄러**를 구현하여, **5분마다** 캐시가 자동으로 최신화되도록 설정했습니다.
...
```
- Redis 캐시는 일반적으로 메모리 관리 효율성을 고려해 **TTL**을 설정합니다. <br>
  이번 캐시 도입에서는 **TTL을 5분 10초**로 설정했습니다. <br> 
  그 이유는 다음과 같습니다. <br> 

- **캐시 스케줄러의 주기가 5분**이기 때문에, 캐시 TTL을 5분 이하로 설정할 경우 **캐시 미스**(Cache miss)가 발생할 수 있습니다. <br>
    이로 인해 순간적으로 DB 조회가 발생하고, 이는 DB 부하를 초래할 가능성이 있습니다. <br> 
    이를 방지하고 서비스를 안정적으로 제공하기 위해, 캐시 TTL을 **5분보다 약간 긴 5분 10초로 설정**하였습니다. <br> 
...
- **'최근 3일간 Top30 콘서트'** 정보를 불러오는 작업에 대해 **k6를 사용하여 2가지 테스트**를 진행했습니다: <br>
  (1) **RDB에서 정보를 검색** <br>
  (2) **Redis에서 정보를 검색** <br> 
...
- **k6 테스트 조건은 다음과 같습니다** <br>
  ...
  (3) **P99 지연시간의 threshold는 1초**로 설정 <br>


**Q. 왜 60초 동안 1000건의 요청으로 테스트했는가?**

A. 인터파크 티켓 사이트를 기준으로 요청 건수를 설정해봤습니다. <br> 
   인터파크 티켓의 2022년 6월 기준 **MAU**(월간 활성 사용자 수)는 **105만 명**입니다. <br> 
   또한, 유사한 사례인 배달의민족의 2019년 기준 MAU가 **800만 명**일 때, RPM(Request Per Minute)은 **3000**이었던 자료를 참고했습니다. <br>
  (참고: https://www.youtube.com/watch?v=MTSn93rNPPE) <br> 

   따라서, 인터파크 티켓은 배달의민족에 비해 MAU가 약 1/8 수준이지만, **메인 페이지**임을 고려하여 요청 건수를 다음과 같이 계산했습니다 <br> 
   ![image](https://github.com/user-attachments/assets/d831eb09-1837-40bf-830c-a4e2631ee402)
   이를 바탕으로 **약 1000 RPM** 정도로 테스트하는 것이 적절하다고 판단했습니다.
...
- **평균 응답 시간은 882.64ms**이었으며, <br>
    90%의 요청이 1.6초 이내, 95%의 요청이 1.85초 이내, 99%의 요청이 2.45초 이내에 응답하였습니다. <br>
