...
- 콘서트 티켓 서비스를 운영하면서, **특정 API들이 빈번하게 DB에 조회**되는 상황이 발생합니다. <br> 
...
- 이를 해결하기 위해, 빈번히 조회되는 API들에 대해 캐시를 도입함으로써 <br> 
  **조회 지연(latency)을 줄이고, 서비스 성능을 개선**하여 사용자 경험을 향상시키고자 합니다. <br> 
...
| **구분**      | **장점**                                                      | **단점**                                                          |
|---------------|---------------------------------------------------------------|-------------------------------------------------------------------|
| **로컬 캐시** | 1. 빠른 데이터 접근 속도를 제공 2. 캐시 서버의 장애로부터 자유롭다 | 1. 분산 환경에서 캐시 동기화가 복잡할 수 있음                    |
| **글로벌 캐시**| 1. 분산 환경에서 Single Source of Truth를 제공                 | 1. 캐시 서버의 장애로 인한 네트워크 장애가 발생할 수 있음        |
