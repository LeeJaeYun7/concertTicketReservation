...
- 단일 인덱스와 복합 인덱스는 각각 이러한 특성을 갖고 있습니다. 
...
- 인덱스의 칼럼을 추가하는 것은 조회 성능을 향상시킬 수 있으나, <br>
  업데이트나 삭제 성능을 저하시키고, 인덱스를 위한 메모리 공간을 더 사용해야 한다는 Trade-off가 있습니다. <br>
  따라서 이러한 Trade-off를 잘 고려해서 의사결정해야 합니다. <br> 


#### (2-2) 테이블의 어떤 필드들을 인덱스로 선택할지
- 인덱스를 위한 필드를 선택할 때는, 보통 카디널리티(Cardinalit)이 높은 필드를 선택하는 것이 권장됩니다. <br>
  카디널리티란 테이블에서 해당 필드에 있는 유니크한 값의 개수를 의미합니다. <br> 
  카디널리티가 높은 필드를 선택하는 것이 좋은 이유는 <br>
  카디널리티가 높은 필드는 검색 범위가 더 좁혀지기 때문에 인덱스의 효과가 더 크게 나타나기 때문입니다. <br>  
